...
public class TravelRouteSqlProvider {
    ...
    public String update(){
        return "UPDATE t_cms_travel_route SET MODIFY_USER_ID=#{modifyUserId},MODIFY_TIME=NOW(),TITLE=#{title}," +
                ...
                "PRODUCT_CODE=#{productCode},PRICE=#{price},STATE=#{state},IMG_URL=#{imgUrl},INTRO=#{intro},NAME=#{name} WHERE id=#{id}";
    }

    public String save(){
        return "INSERT INTO t_cms_travel_route(ID,ADD_USER_ID,ADD_TIME,NAME,TITLE,START_SITE,END_SITE,END_TIME," +
                "START_TIME,DAY,PRODUCT_CODE,PRICE,STATE,IMG_URL,INTRO) VALUES(#{id},#{addUserId},NOW(),name,#{title},#{startSite}," +
